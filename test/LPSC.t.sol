...
contract LPSCTest is Test {
    ...
    function setUp() public {
        ...
        vm.createSelectFork("https://arb-mainnet.g.alchemy.com/v2/KywLaq2zlVzePOhip0BY3U8ztfHkYDmo");
    ...
    function testReplyWithoutVaultInteraction() public {
        ...
        uint256 balanceBefore = IERC20(wethAddressArbitrum).balanceOf(address(lpsc));

        vm.prank(routerAddress);
        lpsc.reply(token, mockAmount, mockSourceChainSelector, mockSender, mockMessageId);
    
        uint256 balanceAfter = IERC20(wethAddressArbitrum).balanceOf(address(lpsc));

        assert(balanceBefore>balanceAfter);
        console.log("Initial Balance :",balanceBefore,"Final Balance: ",balanceAfter);
